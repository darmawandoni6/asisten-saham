...
export function EditBalanceModal({ isOpen, currentBalance, onClose, onSuccess }: EditBalanceModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={open => !open && onClose()}>
      <EditBalanceDialogContent currentBalance={currentBalance} onClose={onClose} onSuccess={onSuccess} />
    </Dialog>
  );
}

function EditBalanceDialogContent({ currentBalance, onClose, onSuccess }: Omit<EditBalanceModalProps, 'isOpen'>) {
      ...
      <form onSubmit={handleSave} className="space-y-4 p-6">
        ...
        <div className="space-y-1.5">
          ...
          <div className="relative">
            <span className="absolute top-1/2 left-3.5 -translate-y-1/2 text-sm font-semibold text-slate-400">Rp</span>
            <Input
              id="balance-input"
              type="text"
              inputMode="numeric"
