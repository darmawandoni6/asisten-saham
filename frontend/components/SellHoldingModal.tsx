...
export function SellHoldingModal({ isOpen, holding, onClose, onSuccess }: SellHoldingModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={open => !open && onClose()}>
      <SellHoldingModalContent key={holding.id} holding={holding} onClose={onClose} onSuccess={onSuccess} />
    </Dialog>
  );
}
        ...
        <div className="grid grid-cols-2 gap-3.5">
          <div className="space-y-1.5">
            ...
            <div className="relative">
              <span className="absolute top-1/2 left-3 -translate-y-1/2 text-xs font-semibold text-slate-400">Rp</span>
              <Input
                id="sell-price-input"
                type="text"
                inputMode="numeric"
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-2">
            <Button
              type="button"
              variant={
                lotNum === Math.max(1, Math.round(holding.lot * 0.25)) && lotNum !== holding.lot ? 'default' : 'outline'
              }
              size="sm"
            ...
            <Button
              type="button"
              variant={
                lotNum === Math.max(1, Math.round(holding.lot * 0.5)) && lotNum !== holding.lot ? 'default' : 'outline'
              }
              size="sm"
