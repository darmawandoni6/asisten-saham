...
import { Holding, AICopilotAnalysisResult } from "@/types";
...
export function AICopilotPanel({ holding, onClose }: AICopilotPanelProps) {
  ...
  useEffect(() => {
    ...
    async function loadAnalysis() {
      try {
        const res = await api.analyzeStock(holding.ticker, selectedProvider);
        if (isMounted) {
  ...
  }, [holding.ticker, selectedProvider]);
  ...
  const handleSwitchProvider = (provider: "gemini" | "opencode_zen") => {
  ...
  const activeBadgeLabel = data?.provider === "opencode_zen" 
    ? "OpenCode Zen AI" 
    : "Google Gemini AI";
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl relative">
      ...
      {onClose && (
        <button
          type="button"
          onClick={onClose}
          className="absolute top-5 right-5 w-8 h-8 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-500 hover:text-slate-800 flex items-center justify-center transition-colors z-10 cursor-pointer"
        >
          <X className="w-4 h-4" />
        </button>
      )}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-5 border-b border-slate-100">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-emerald-100 flex items-center justify-center text-emerald-700 shadow-2xs shrink-0">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className={`text-[10px] px-2 py-0.5 rounded font-bold flex items-center gap-1 border ${
                ...
                {selectedProvider === "opencode_zen" ? <Bot className="w-3 h-3 text-indigo-600" /> : <Sparkles className="w-3 h-3 text-emerald-600" />}
                {activeBadgeLabel}
              </span>
            </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            disabled={isLoading || isRefreshing}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold transition-colors disabled:opacity-50 cursor-pointer"
          >
            <RefreshCw className={`w-3.5 h-3.5 ${isRefreshing ? "animate-spin text-emerald-600" : ""}`} />
            <span>{isRefreshing ? "Menganalisis..." : "Analisis Ulang"}</span>
          </button>
        </div>
      </div>
      ...
      {isLoading && (
        <div className="py-16 text-center">
          ...
          <h4 className="text-sm font-bold text-slate-800">Menghubungkan ke {selectedProvider === "opencode_zen" ? "OpenCode Zen AI" : "Google Gemini AI"}...</h4>
              ...
              <div className="space-y-3 flex-1">
                <div>
                  <div className="flex items-center gap-2">
                    <h4 className="text-sm font-bold text-amber-900">
                      API Key {selectedProvider === "opencode_zen" ? "OpenCode Zen" : "Google Gemini"} Belum Terpasang
                    </h4>
                  ...
                  <p className="text-xs text-amber-800 mt-1 leading-relaxed">
                    {data.message || `Fitur AI Copilot saat ini belum tersedia karena API Key ${selectedProvider === "opencode_zen" ? "OpenCode Zen" : "Google Gemini"} belum dikonfigurasi.`}
                  </p>
                </div>
                ...
                <div className="p-3.5 rounded-lg bg-white/90 border border-amber-200 text-xs text-slate-700 space-y-2">
                  <div className="font-semibold text-slate-800 flex items-center gap-1.5">
                    ...
                    <span>Langkah Mudah Konfigurasi {selectedProvider === "opencode_zen" ? "OpenCode Zen" : "Google Gemini"}:</span>
                  </div>
                  {selectedProvider === "opencode_zen" ? (
                    <ol className="list-decimal list-inside space-y-1 text-slate-600 text-[11px] leading-relaxed">
                      <li>
                        Buka dashboard OpenCode:{" "}
                        <a 
                          href="https://opencode.ai" 
                          target="_blank" 
                          rel="noreferrer"
                          className="text-indigo-700 underline font-semibold inline-flex items-center gap-0.5"
                        >
                          opencode.ai <ExternalLink className="w-2.5 h-2.5" />
                        </a>.
                      </li>
                      <li>Masuk ke menu <strong>API Keys</strong> dan salin key Anda.</li>
                      <li>
                        Simpan di file <code className="px-1.5 py-0.5 bg-slate-100 rounded text-slate-800 font-mono text-[10px]">backend/.env</code>:
                        <div className="mt-1 p-2 bg-slate-900 text-indigo-300 rounded font-mono text-[11px]">
                          OPENCODE_API_KEY=zen_your_key_here
                        </div>
                      </li>
                      <li>Klik tombol <strong>Coba Analisis Lagi</strong> di bawah.</li>
                    </ol>
                  ) : (
                    <ol className="list-decimal list-inside space-y-1 text-slate-600 text-[11px] leading-relaxed">
                      <li>
                ...
                <div className="pt-1 flex items-center gap-3">
                  <button
                    type="button"
                    onClick={handleRefresh}
                    className="px-4 py-2 rounded-xl bg-amber-600 hover:bg-amber-500 text-white font-semibold text-xs transition-colors flex items-center gap-2 shadow-2xs cursor-pointer"
                  >
                    <RefreshCw className="w-3.5 h-3.5" />
                    <span>Coba Analisis Lagi</span>
                  </button>
                  <span className="text-[11px] text-amber-700">
                    Sistem otomatis mendeteksi begitu key tersimpan.
                  </span>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-3 flex-1">
                <div>
                  ...
                  <p className="text-xs text-rose-800 mt-1 leading-relaxed">
                    {data.message || "AI belum dapat menjawab saat ini karena limit token atau kuota harian telah habis."}
                  </p>
                  <p className="text-[11px] text-rose-600 mt-1">
                    {data.detail || "Anda juga bisa beralih ke provider AI alternatif menggunakan tombol di pojok kanan atas."}
                  </p>
                </div>
                ...
                <div className="pt-1 flex items-center gap-3">
                  <button
                    type="button"
                    onClick={handleRefresh}
                    className="px-4 py-2 rounded-xl bg-rose-600 hover:bg-rose-500 text-white font-semibold text-xs transition-colors flex items-center gap-2 shadow-2xs cursor-pointer"
                  >
                    <RefreshCw className="w-3.5 h-3.5" />
        ...
        <>
          ...
          <div className="mt-5 p-4 rounded-xl bg-slate-50 border border-slate-200 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <div className="flex items-center gap-3 mt-1">
                ...
                <span className="text-xs text-slate-500">
                  Confidence: <span className="font-bold text-slate-900 font-mono">{data.confidence || 90}%</span>
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4 text-xs font-mono bg-white px-4 py-2.5 rounded-lg border border-slate-200 shadow-2xs">
              <div>
                ...
                <span className="font-bold text-slate-900">Rp {formatNumber(Math.round(data.currentPrice || 0))}</span>
              </div>
              <div className="h-6 w-px bg-slate-200" />
              <div>
                <span className="text-slate-400 text-[10px] block font-sans">Avg Beli</span>
                <span className="font-bold text-slate-700">Rp {formatNumber(Math.round(data.avgPrice || 0))}</span>
              </div>
              <div className="h-6 w-px bg-slate-200" />
              <div>
          ...
          <div className="mt-5">
            <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">
              Rasional & Evaluasi Emosi Pasar ({activeBadgeLabel})
            </h4>
            <div className="p-4 rounded-xl bg-slate-50 border border-slate-200">
              <MarkdownText content={data.rationale || ""} className="text-xs leading-relaxed" />
            </div>
          </div>
              ...
              <div className="space-y-2">
                {data.actionItems.map((item: string, idx: number) => (
                  ...
                  >
                    ...
                    <MarkdownText content={item} className="text-xs leading-relaxed flex-1" />
                  </div>
                ))}
              </div>
            </div>
