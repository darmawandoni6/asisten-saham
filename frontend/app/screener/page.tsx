...
export default function ScreenerPage() {
  ...
  const mapScreenerItem = (r: any): ScreenerItem => ({
    ...
    buyArea:
      r.buy_area ||
      `Rp ${formatNumber(r.support)} – Rp ${formatNumber(r.price)}`,
    ...
    potentialGainPct:
      r.potential_gain_pct ||
      (r.price > 0
        ? Math.round(((r.resistance - r.price) / r.price) * 100)
        : 0),
    potentialRiskPct:
      r.potential_risk_pct ||
      (r.price > 0
        ? Math.round(((r.price - (r.stop_loss || r.support)) / r.price) * 100)
        : 0),
    support: r.support,
  ...
  const handleAnalyzeCustomTicker = async () => {
    ...
    try {
      ...
      if (res && res.ticker) {
        ...
        setItems((prev) => [
          newItem,
          ...prev.filter((p) => p.ticker !== res.ticker),
        ]);
        setActiveTab("ALL");
  ...
  const handleSort = (field: SortField) => {
    if (sortField === field) {
      ...
      setSortField(field);
      setSortDirection(
        field === "ticker" || field === "strategy" ? "asc" : "desc",
      );
    }
  };
  ...
  const filteredItems = items.filter((item) => {
    ...
    const matchesPrice =
      maxPriceFilter === null || item.price <= maxPriceFilter;
    ...
    return (
      <th
        ...
        className={`py-3.5 px-3 select-none cursor-pointer hover:bg-slate-100/80 transition-colors ${
          isActive
            ? "text-emerald-800 font-black bg-emerald-50/50"
            : "text-slate-600 font-bold"
        } ${align === "right" ? "text-right" : "text-left"}`}
        title={
          tooltip
            ? `${label}: ${tooltip} (Klik untuk mengurutkan)`
      ...
      <div className="p-6 space-y-6 max-w-7xl mx-auto w-full">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-500">
                ...
                <code className="bg-slate-100 px-1.5 py-0.5 rounded text-slate-700 font-mono font-semibold">
                  BREN
                </code>
                ,{" "}
                <code className="bg-slate-100 px-1.5 py-0.5 rounded text-slate-700 font-mono font-semibold">
                ...
                ) untuk langsung dianalisis &amp; dimasukkan ke daftar
                rekomendasi.
              </p>
            </div>
              ...
              {maxPriceFilter !== null ? (
                <span>
                  ...
                  <strong className="text-emerald-700 font-mono">
                    ≤ Rp {formatNumber(maxPriceFilter)}
                  </strong>{" "}
                  (≤ Rp {formatNumber(maxPriceFilter * 100)}/lot)
                </span>
              ) : (
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3 rounded-2xl bg-white border border-slate-200 shadow-2xs">
            ...
            <div className="flex items-center justify-between sm:justify-end gap-3 pt-2 sm:pt-0 border-t sm:border-t-0 border-slate-100">
              <span className="text-[11px] text-slate-400 font-medium">
                Menampilkan{" "}
                <strong className="text-slate-700">{sortedItems.length}</strong>{" "}
                saham
              </span>
        ...
        {sortedItems.length > 0 ? (
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 text-xs">
                    ...
                    <div className="p-3.5 rounded-xl bg-amber-50/40 border border-amber-200/80 space-y-1.5">
                      ...
                      <div className="text-[11px] text-amber-900/80 font-medium pt-1">
                        👉{" "}
                        <em>
                          Disiplin entry hanya saat trigger terkonfirmasi.
                        </em>
                      </div>
                    </div>
                      ...
                      <div className="grid grid-cols-2 gap-2 text-[11px] font-mono">
                        ...
                        <div className="p-2 rounded-lg bg-emerald-50/60 border border-emerald-200/60">
                          ...
                          <strong className="text-emerald-900 font-bold">
                            Rp{" "}
                            {formatNumber(item.targetPrice || item.resistance)}{" "}
                ...
                <table className="w-full text-left text-xs">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-200 text-slate-500 font-bold uppercase tracking-wider text-[10px]">
                      ...
                      <th className="py-3.5 px-3 font-bold text-slate-600">
                        Area Beli Disarankan
                      </th>
                      ...
                      {renderSortTh(
                        ...
                        "score",
                        "left",
                        "AI Technical Score (0-100). Tingkat kematangan setup teknikal berbasis konvergensi MA20/MA50, RSI, dan jarak support/resist.",
                      )}
                      <th className="py-3.5 px-3 text-right font-bold text-slate-600">
                        Detail
                      </th>
                    </tr>
                  </thead>
                        ...
                        <React.Fragment key={item.ticker}>
                          ...
                          >
                            ...
                            <td className="py-3.5 px-3 font-mono font-bold text-emerald-800 text-[11px]">
                              Rp{" "}
                              {formatNumber(
                                item.targetPrice || item.resistance,
                              )}
                            </td>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                                  ...
                                  <div className="p-3 rounded-xl bg-amber-50/50 border border-amber-200 space-y-1">
                                    <div className="font-bold text-amber-950 flex items-center gap-1 text-[11px] uppercase tracking-wider">
                                      <Eye className="w-3.5 h-3.5 text-amber-700" />
                                      <span>
                                        Wajib Dipantau Besok (09:00 WIB):
                                      </span>
                                    </div>
                                    <p className="text-slate-700 leading-relaxed">
                                      {item.watchTrigger}
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center text-slate-400 shadow-2xs">
            ...
            <p className="text-xs text-slate-500 mt-1 mb-5 max-w-md mx-auto">
              Klik tombol &quot;Scan EOD (Top 10)&quot; untuk memindai 35+ saham
              teraktif BEI dan menghasilkan 10 rekomendasi terbaik pasca
              penutupan pasar.
            </p>
            <button
              type="button"
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-2xs">
          ...
          <p className="text-xs text-slate-500 leading-relaxed">
            Daftar ini adalah <strong>watchlist intelijen terkurasi</strong>{" "}
            pasca penutupan pasar pukul 17:30 WIB. Setiap saham dilengkapi
            alasan teknikal objektif (*Why Buy*), hal wajib dipantau besok pagi
            (*Watch Trigger*), serta kalkulasi rasio *Risk/Reward* (RRR). Jangan
            langsung melakukan pembelian sebelum syarat pantauan jam 09:00 WIB
            ...
            <div className="p-4 sm:p-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    Panduan formula teknikal, arti AI Score, dan matematika
                    probabilitas Risk:Reward
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="p-5 overflow-y-auto space-y-4 text-xs">
              ...
              <div className="p-4 rounded-xl bg-emerald-50/60 border border-emerald-200 space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[11px] font-bold text-emerald-800">
                    Matematika Ketahanan Modal
                  </span>
                </div>
                <strong className="block text-slate-900 text-sm">
                  Kunci Profit Konsisten: Mengapa RRR &ge; 1 : 2.0 Sangat
                  Krusial?
                </strong>
                <p className="text-slate-700 leading-relaxed">
                  RRR membandingkan{" "}
                  <strong>
                    berapa rupiah risiko yang Anda korbankan (Stop Loss)
                  </strong>{" "}
                  terhadap{" "}
                  <strong>
                    berapa rupiah potensi keuntungan yang Anda incar (Target TP)
                  </strong>
                  .
                </p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-1">
                  <div className="p-2.5 rounded-lg bg-white border border-emerald-200 text-[11px] space-y-1">
                    ...
                    <span className="text-emerald-800 font-medium block text-[10px]">
                      Contoh: Beli 1.000, SL 950 (-5%), TP 1.100 (+10%) &rarr;{" "}
                      <strong>RRR = 1 : 2.0</strong>
                    </span>
                  </div>

                  <div className="p-2.5 rounded-lg bg-white border border-emerald-200 text-[11px] space-y-1">
                    <span className="font-bold text-slate-900 block">
                      🏆 Simulasi Win-Rate 40%:
                    </span>
                    <p className="text-slate-600">
                      Dari 10 trade: <strong>6x Rugi (-Rp 300)</strong> vs{" "}
                      <strong>4x Cuan (+Rp 400)</strong>.
                    </p>
              ...
              <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[11px] font-semibold text-slate-500">
                    Filter Probabilitas Statistik
                  </span>
                </div>
                <p className="text-slate-600 leading-relaxed">
                  AI Score mengukur <strong>kematangan kondisi teknikal</strong>{" "}
                  saat penutupan bursa (posisi harga terhadap MA20/MA50,
                  momentum RSI, dan jarak ke support/resistance).
                </p>
                <div className="p-2.5 rounded-lg bg-amber-50 border border-amber-200 text-[11px] text-amber-950 leading-snug">
                  ⚠️ <strong>PENTING:</strong> AI Score tinggi{" "}
                  <strong>BUKAN jaminan pasti naik besok</strong>, melainkan
                  indikator bahwa secara historis saham ini berada pada titik
                  probabilitas rebound/breakout yang lebih menguntungkan.
                  Disiplin tunggu trigger pembukaan pukul 09:00 WIB.
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-xs pt-1">
                  ...
                  <div className="p-2.5 rounded-lg bg-white border border-blue-200">
                    ...
                    <span className="text-slate-600 text-[11px] leading-snug block mt-0.5">
                      Sinyal Baik (Tunggu konfirmasi volume &amp; candle hijau
                      esok pagi).
                    </span>
                  </div>
                  <div className="p-2.5 rounded-lg bg-white border border-slate-200">
                    <strong className="block text-slate-700 font-bold">
                      &lt; 75 (Abu-abu)
                    </strong>
                    <span className="text-slate-600 text-[11px] leading-snug block mt-0.5">
              ...
              <div className="space-y-3 pt-1">
                ...
                <div className="p-3.5 rounded-xl bg-purple-50/50 border border-purple-200 space-y-1.5">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs font-semibold text-purple-700">
                      RSI &lt; 35 • Support Mayor
                    </span>
                  </div>
                  <strong className="block text-slate-900 text-sm">
                    Jenuh Jual Ekstrem (Buy on Weakness)
                  </strong>
                  <p className="text-slate-600 leading-relaxed">
                  ...
                  <p className="text-purple-900 font-medium pt-1">
                    👉 <strong>Rekomendasi Aksi:</strong> Beli bertahap saat
                    candle hijau/pantulan reversal muncul. Pasang SL ketat di
                    bawah support.
                  </p>
                </div>
                ...
                <div className="p-3.5 rounded-xl bg-blue-50/50 border border-blue-200 space-y-1.5">
                  ...
                  <p className="text-slate-600 leading-relaxed">
                    Harga menembus dan bertahan di atas MA20 dengan volume
                    aktif. Fase sideways selesai dan tren akselerasi dimulai.
                  </p>
                  <p className="text-blue-900 font-medium pt-1">
                    👉 <strong>Rekomendasi Aksi:</strong>{" "}
                    <em>Buy on Momentum</em> untuk menunggangi tren akselerasi
                    jangka pendek.
                  </p>
                </div>
                ...
                <div className="p-3.5 rounded-xl bg-amber-50/50 border border-amber-200 space-y-1.5">
                  ...
                  <p className="text-slate-600 leading-relaxed">
                    Saham berfundamental kuat (Blue Chip/LQ45) yang
                    berkonsolidasi stabil di atas garis penopang MA50.
                  </p>
                  <p className="text-amber-900 font-medium pt-1">
                    👉 <strong>Rekomendasi Aksi:</strong> Cicil beli santai
                    (*DCA*) untuk portofolio investasi jangka menengah-panjang.
                  </p>
                </div>
              </div>
            </div>
